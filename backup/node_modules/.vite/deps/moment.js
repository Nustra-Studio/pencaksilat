...
function handleStrictParse2(weekdayName, format2, strict) {
...
function localeWeekdaysParse(weekdayName, format2, strict) {
  ...
  if (this._weekdaysParseExact) {
    return handleStrictParse2.call(this, weekdayName, format2, strict);
  }
  if (!this._weekdaysParse) {
    this._weekdaysParse = [];
    this._minWeekdaysParse = [];
...
function addSubtract(mom, duration, isAdding, updateOffset) {
  ...
  if (days2) {
    set2(mom, "Date", get(mom, "Date") + days2 * isAdding);
  }
  if (milliseconds2) {
...
function startOf(units) {
  ...
  switch (units) {
    ...
    case "hour":
      ...
      time -= mod2(time + (this._isUTC ? 0 : this.utcOffset() * MS_PER_MINUTE), MS_PER_HOUR);
      break;
    case "minute":
      time = this._d.valueOf();
...
function endOf(units) {
  ...
  switch (units) {
    ...
    case "hour":
      time = this._d.valueOf();
      time += MS_PER_HOUR - mod2(time + (this._isUTC ? 0 : this.utcOffset() * MS_PER_MINUTE), MS_PER_HOUR) - 1;
      break;
    case "minute":
      time = this._d.valueOf();
...
function get2(format2, index, field, setter) {
...
function listMonthsImpl(format2, index, field) {
  ...
  format2 = format2 || "";
  if (index != null) {
    return get2(format2, index, field, "month");
  }
  var i;
  var out = [];
  for (i = 0; i < 12; i++) {
    out[i] = get2(format2, i, field, "month");
  }
  return out;
}
function listWeekdaysImpl(localeSorted, format2, index, field) {
  ...
  if (index != null) {
    return get2(format2, (index + shift) % 7, field, "day");
  }
  var i;
  var out = [];
  for (i = 0; i < 7; i++) {
    out[i] = get2(format2, (i + shift) % 7, field, "day");
  }
  return out;
}
...
function relativeTime2(posNegDuration, withoutSuffix, locale2) {
...
function humanize(withSuffix) {
  ...
  var output = relativeTime2(this, !withSuffix, locale2);
  if (withSuffix) {
...
function toISOString2() {
  ...
  var seconds2 = abs2(this._milliseconds) / 1e3;
  var days2 = abs2(this._days);
  var months2 = abs2(this._months);
